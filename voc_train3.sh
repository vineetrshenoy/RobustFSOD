...
#
#
#CONFIG_PATH=configs/voc/mfdc_gfsod_novel1_1shot_seed0.yaml
#OUTPUT_DIR=checkpoints/voc/vocR101-1/mfdc_gfsod_novel1/tfa-like/1shot_seed0
#
#CUDA_VISIBLE_DEVICES=3,4,5,6 python3 train_net.py --num-gpus 4 --eval-only --config-file ${CONFIG_PATH}                            \
...
do
    ...
    do
        ...
        CUDA_VISIBLE_DEVICES=0,1,2,3 python3 train_net.py --num-gpus 4 --config-file ${CONFIG_PATH}                            \
            --opts MODEL.WEIGHTS ${BASE_WEIGHT} OUTPUT_DIR ${OUTPUT_DIR}                     \
                   TEST.PCB_MODELPATH ${IMAGENET_PRETRAIN_TORCH}
        rm ${CONFIG_PATH}
